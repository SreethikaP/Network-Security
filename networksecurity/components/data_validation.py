...
class DataValidation:
        ...
        try:
            ...
            for column in required_numerical_columns:
                if column not in dataframe.columns:
                    logging.warning(f"Column {column} is missing in the dataframe.")
                    return False

                if not pd.api.types.is_numeric_dtype(dataframe[column]):
                    logging.warning(f"Column {column} is not of numeric type.")
                    return False

            logging.info("All required numerical columns are valid.")
            return True
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = f"Train dataframe does not contain valid numerical columns.\n"

            # Validate numerical columns in test dataframe
            status = self.validate_numerical_columns(dataframe=test_dataframe)
            if not status:
                error_message = f"Test dataframe does not contain valid numerical columns.\n"
